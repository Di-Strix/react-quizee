...
const handlers = {
  ...
  [TYPES.SAVE_QUESTIONS]: (state, { payload }) => ({
    ...state,
    questions: payload.questions,
    caption: payload.caption,
  }),
  [TYPES.LOAD_RESULTS]: state => ({
    ...state,
  ...
  [TYPES.SHOW_RESULTS]: (state, { payload }) => ({
    ...state,
  ...
  [TYPES.SET_QUIZEE_ID]: (_, { payload }) => ({ ...initialState(), quizeeId: payload }),
  [TYPES.ERROR]: (state, { payload }) => ({ ...state, screen: SCREENS.ERROR, text: payload }),
  [TYPES.FOOTER_SET_NEXT_BUTTON_STATE]: (state, { payload }) => ({ ...state, Footer: { active: payload } }),
  [TYPES.UPDATE_TRANSITION_KEY]: state => ({ ...state, transitionKey: getKey() }),
  DEFAULT: state => state,
}
